...
def multiclass_log_probs(config, pred, targ, shift=False, eps=torch.finfo(torch.float32).eps, exact_match=False, **kwargs):
    ...
    if 'mask' in kwargs.keys() and kwargs['mask']:
        _, topk_indices = torch.topk(pred, 10, dim=-1)
        ce_mask = []
        for i in range(targ.shape[-1]):
            dim, device = pred.shape[-1], pred.device
            t_mask = torch.zeros(dim).to(device)
            t_mask[topk_indices[0][i]] = 1. # only use top10 logits of tokens
            if targ[0][i] != 0: # ans token also should be included
                t_mask[targ[0][i]] = 1.
            ce_mask.append(t_mask)
        if len(ce_mask) > 0:
            ce_mask = torch.stack(ce_mask, dim=0).unsqueeze(0)
            pred = pred * ce_mask
    ...
    if "inner_sent" in kwargs or "inner_per" in kwargs:
        ...
        masked_log_probs_ = unmasked_log_probs * mask.float()
    ...
    return {
        ...
        "n_tokens": n_tokens,
        "nll": nll,
        "pred": pred_ids.cpu().numpy(),
        "targ": targ.cpu().numpy(),
        'mask': mask.cpu(),
        'logits': pred,
        'masked_log_probs':masked_log_probs_,
    }
