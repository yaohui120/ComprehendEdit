...
def get_model(config):
    if config.model_name == "blip2" or config.model_name == "minigpt4":
        ...
        device_map = get_model_key(config.name)
    ...
    if config.model_class == "BertClassifier":
        ...
        if config.model_parallel:
            model.ln_vision.to(config.device)
            model.visual_encoder.to(config.device)
            model.Qformer.to(config.device)
            model.opt_proj.to(config.device)
        ...
        if config.model_parallel:
            ...
            model.visual_encoder.to(config.device)
            model.Qformer.to(config.device)
            model.llama_proj.to(config.device)
    else:
