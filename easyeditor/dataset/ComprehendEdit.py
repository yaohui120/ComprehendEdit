...
class ComprehendEdit(BaseDataset):
    def __init__(self, data_dir: str, size:  typing.Optional[int] = None, config=None, *args, **kwargs):
        ...
        if self.mode == 'train' and ('diverse' in kwargs.keys() and kwargs['diverse']):
            ...
            path = os.path.join(save_dir, 'ComprehendEdit_{}_{}_diverse.pth'.format(self.mode, config.tokenizer_class))
        else:
        ...
        if self.config.alg.lower() in ['ike', 'ft', 'serac_multi', 'mend', 'hice'] and self.mode != 'train' and self.topk != -1 and size is None:
            self.test_ = True
        else:
            self.test_ = False

        if self.test_:
            path = os.path.join(save_dir, 'ComprehendEdit_inner_{}.pth'.format(config.tokenizer_class))
            if os.path.exists(path):
            ...
            path = os.path.join(save_dir, 'ComprehendEdit_ori_right_{}.pth'.format(config.tokenizer_class))
            if os.path.exists(path):
                self.ori_right = torch.load(path)
            else:
                self.ori_right = []
                # right_data = [json.loads(q) for q in open(os.path.join(data_dir,'ComprehendEdit_ori_right.json'), "r")][0]
