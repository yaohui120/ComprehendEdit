...
class ALinear(nn.Linear):
    def __init__(self, in_dim, out_dim, bias=True, fc=None, new_weight=None):
        super(ALinear, self).__init__(in_dim, out_dim, bias)
        self.type = fc.weight.dtype
        self.weight.data = fc.weight.clone()
        if fc.bias is not None:
            self.bias.data = fc.bias.clone()
        else:
            self.bias = None
        self.new_weight = new_weight.to(self.type)
    
    def forward(self, input):
        return F.linear(input, self.weight, self.bias) + F.linear(input, self.new_weight)

class MEND_multigpu(EditableModel):
    def get_shape(self, p):
        # We need to flip the shapes since OpenAI gpt2 uses convs instead of linear
        return (
            p.shape
    ...
    def __init__(self, model, config, model_constructor, mend=None, edit_lrs=None):
        ...
        if mend is None:
            ...
            if self.config.model_parallel:
                # self.mend.to(deque(self.model.parameters(), maxlen=1)[0].device)
                if len(self.config.gpu_used_id) <= 1:
                    self.mend.to(self.config.device)
                else:
                    self.mend.to(self.config.gpu_used_id[-1])
            else:
                self.mend.to(self.config.device)
        else:
            self.mend = mend
    ...
    # only test for blip2 and minigpt4. for other may be changed
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        # 得到edited_model后，就不用self.model提取特征了，所以可以编辑的时候先恢复再这样
        # 如果编辑后还需要self.model提取特征，那时候self.model还得再恢复成original状态
        pset = set(self.config.inner_params)
        all_modules = [i for i in list(pset)]
        modules = [i[:-7] for i in all_modules if 'weight' in i]
        if self.ori_module != {}:
            for name, module in self.model.named_modules():
                if name in modules:
                    self._set_module(self.model, name, self.ori_module[name])
        ...
        if self.config.shared:
            ...
            transformed_factors = {
                n: self.mend[str(tuple(self.get_shape(p)))](
                    p.__x__, p.__delta__, param_idx(n, p) # p.__delta__[0].T @ p.__x__[0] = p.grad
                )
                for n, p in _inner_params(
                    self.model.named_parameters(), self.config.inner_params
                )
        ...
        for name, module in edited_model.named_modules():
            if name in modules:
                self.ori_module[name] = module
                (input_dim, output_dim) = module.weight.shape
                self._set_module(edited_model, name, ALinear(input_dim, output_dim, fc=module, new_weight=updates[name+'.weight']))
