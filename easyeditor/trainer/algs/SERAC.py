    ...
    def __init__(self, model, config, model_constructor, classifier=None, classifier_tok=None,
        ...
        if replacement is None:
            ...
            if self.config.freeze_cntr:
                ...
                from ...util.tools import get_device_map, get_model_key
                device_map = get_model_key(config.small_name)
                device_map = get_device_map(device_map, config)
                if config.model_class == "BertClassifier":
                    self.replacement = BertClassifier(config.small_name)
                elif config.model_name == "blip2":
                    if "opt" in config.name:
                        from transformers import OPTForCausalLM
                        self.replacement = OPTForCausalLM.from_pretrained(config.small_name, device_map=device_map if config.model_parallel else None)
                elif config.model_name == "minigpt4":
                    from transformers import LlamaForCausalLM
                    self.replacement = LlamaForCausalLM.from_pretrained(config.small_name, device_map=device_map if config.model_parallel else None)
        ...
        # self.replacement.model.to(config.device)
        self.language_projection.to(config.device)
        ...
        if self.config.freeze_cntr:
            ...
            if self.config.model_name == "blip2":
                ...
                if image is not None:
                    ...
                    query_tokens = self.model.query_tokens.expand(image_embeds.shape[0], -1, -1).to(image.device)
                    ...
                    targets = torch.cat([empty_targets, targets.to(image.device)], dim=1)
